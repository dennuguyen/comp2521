...
{
	if (t == NULL) // empty tree case
		return;

	Queue q = newQueue();
	QueueJoin(q, t); // push root node into queue
	while (!QueueIsEmpty(q))
	{
		BSTNode *tmp = QueueLeave(q); // dequeue and search its child nodes

		showBSTreeNode(tmp);

		if (tmp->left) // enqueue left child node
			QueueJoin(q, tmp->left);

		if (tmp->right) // enqueue right child node
...
{
	/*
	// recursive solution - worst: O(2^n)
	// can reduce to O(n) with memoisation but ceebs
	if (t == NULL)
		return 0;
	...
	*/
	...
	while (!QueueIsEmpty(q))
	{
		...
		// leaf node is any node with empty left AND right subtrees
