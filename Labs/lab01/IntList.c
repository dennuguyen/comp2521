...
void IntListInsertInOrder(IntList L, int v)
{
	...
	// fail function if L is not sorted
	...
	{
		// compare consecutive paired nodes of L
		for (struct IntListNode *curr = L->first;
			 curr->next != NULL; curr = curr->next)
			if (curr->data <= v && v <= curr->next->data)
			{
				struct IntListNode *n = newIntListNode(v);
				struct IntListNode *tmp = curr->next;
				curr->next = n;
				n->next = tmp;
				L->size++;
				return;
